...
class admin_member extends MX_Controller  {
	...
	function __construct()
	{
		...
		$this->load->model("admin_member",'member');
	...
	{	
		...
		$jum_record = $this->member->getTotal($this->table,"member",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->member->getList($this->table,"member",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
			foreach($query->result_array() as $key => $r)
			{
				...
				$list[$key] 				= $r;
				$list[$key]['no']			= $no;
				$list[$key]['create_date'] 	= date("d/m/Y H:i",strtotime($r['date']));
		...
		if(is_numeric($id)){
			...
			if($q->num_rows() > 0){
				foreach($q->result_array() as $r){
					...
					$fieldRow['password'] = base64_decode($r['password']);
					$list[] = $fieldRow;
				}
			}else{
	...
	function submit()
	{
		...
		foreach ($fields as $field) {
			if($field!='nip'&&$field!='file_image'&&$field!='publish'&&$field!='create_date' && $field!='modify_date' && $field!='modify_user_id' && $field!='date2' && $field!='modify_user_id' && $field!='user_id'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				$this->member->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{
				$id_term = $this->member->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
