...
class admin_pengadaan extends MX_Controller  {
	
	var $table = "pengadaan";
	var $table_alias = "Pengadaan";
	var $uri_page = 6;
	var $per_page = 25;
	...
	function __construct()
	{
		...
		$this->load->model("admin_ref_pengadaan/model_ref_pengadaan",'ref_pengadaan');
	...
	{	
		...
		$jum_record = $this->pengadaan->getTotal($this->table,"ref_pengadaan",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->pengadaan->getList($this->table,"ref_pengadaan",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->pengadaan->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
				foreach($q->result_array() as $r){
					...
					$qref3 = $this->db->get('ref_pengadaan');
					$ref3 = "<option value=''> - select value - </option>";
					foreach ($qref3->result() as $val) {
						$selected = $val->id == $r['ref_pengadaan_id'] ? "selected='selected'" : "";	
					...
					$qref4 = $this->db->get('propinsi');
					$ref4 = "<option value=''> - select value - </option>";
					foreach ($qref4->result_array() as $val) {
						$selected = "";	
						$ref4 .= "<option value='".$val['id']."' ".$selected.">".$val['title']."</option>";
					}
	...
	function submit()
	{
		...
		$postData['file_image'] = $file_image_name;
		# End the image
		...
		unset($fields['views']);
		foreach ($fields as $field) {
			if($field!='views' && $field!='publish' && $field!='file_image' && $field!='create_date' && $field!='modify_date' && $field!='modify_user_id' && $field!='date2' && $field!='modify_user_id' && $field!='user_id'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				$this->pengadaan->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{
				$id_term = $this->pengadaan->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->pengadaan->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->pengadaan->getList($this->table,null,null,null);
		$list = array();
