...
class admin_engine_metodeevaluasi extends MX_Controller  {
	
	var $table = "engine_metodeevaluasi";
	var $table_alias = "Metode Evaluasi";
	var $uri_page = 6;
	var $per_page = 25;
	...
	function __construct()
	{
		...
		$this->load->model("admin_engine_metodeevaluasi",'engine_metodeevaluasi');
	...
	{	
		...
		$jum_record = $this->engine_metodeevaluasi->getTotal($this->table,"engine_metodeevaluasi",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->engine_metodeevaluasi->getList($this->table,"engine_metodeevaluasi",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->engine_metodeevaluasi->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
	...
	function submit()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($_POST['id_first'] > 0)
			{
				...
				$this->engine_metodeevaluasi->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{
				unset($postData['id_first']);
				$id_term = $this->engine_metodeevaluasi->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->engine_metodeevaluasi->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->engine_metodeevaluasi->getList($this->table,null,null,null);
		$list = array();
