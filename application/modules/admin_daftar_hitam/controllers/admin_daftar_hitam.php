...
class admin_daftar_hitam extends MX_Controller  {
	
	var $table = "daftar_hitam";
	var $table_alias = "Daftar Hitam";
	var $uri_page = 6;
	var $per_page = 25;
	...
	function __construct()
	{
		...
		//$this->load->model("admin_ref_daftar_hitam/model_ref_daftar_hitam",'ref_daftar_hitam');
	...
	{	
		...
		$jum_record = $this->daftar_hitam->getTotal($this->table,"ref_daftar_hitam",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->daftar_hitam->getList($this->table,"ref_daftar_hitam",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->daftar_hitam->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
				foreach($q->result_array() as $r){
					...
					// $qref3 = $this->db->get('ref_daftar_hitam');
					// $ref3 = "<option value=''> - select value - </option>";
					// foreach ($qref3->result() as $val) {
					// 	$selected = $val->id == $r['ref_daftar_hitam_id'] ? "selected='selected'" : "";	
					...
					$fieldRow['create_date'] = !empty($fieldRow['create_date']) ? date('d F Y H:i:s',strtotime($fieldRow['create_date'])) : '';
					$fieldRow['ref2'] = $ref2;
					//$fieldRow['ref3'] = $ref3;
					$fieldRow['ref4'] = $ref4;
					$fieldRow['ref5'] = $ref5;
			...
			#end notification
			$view = $this->daftar_hitam->viewBlacklist($id);
			$data = array(
					  'base_url' => base_url(),
					  'admin_url' => base_url(),
					  'notif'=>$notif,
	...
	function submit()
	{
		...
		unset($fields['date2']);
		foreach ($fields as $field) {
			if($field!='is_delete'&&  $field!='deleted_at' && $field!='show_createdate'&&$field!='logs' && $field!='keterangan' && $field!='deleted_at' && $field!='is_delete' && $field!='publish' && $field!='file_image' && $field!='file_pencabutan' && $field!='create_date' && $field!='modify_date' && $field!='modify_user_id' && $field!='date2' && $field!='modify_user_id' && $field!='user_id'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				unset($postData['rekanid']);
				$rekanid = $this->input->post('rekanid');
				$this->daftar_hitam->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old,$rekanid);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{
				unset($postData['rekanid']);
				$rekanid = $this->input->post('rekanid');
				$id_term = $this->daftar_hitam->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old,$rekanid);
	...
	function cron(){
		...
		$this->db->update('daftar_hitam',$data);
		echo "cron running completed";
	}
	
	function delete($id=0)
	{
		$del_status = $this->daftar_hitam->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->daftar_hitam->getList($this->table,null,null,null);
		$list = array();
	...
	function getRkn(){
		...
		if($search!=""){
			...
			$data['data'] = $this->daftar_hitam->getBlacklist($search,$id);
			//$data['data'] = array("isi"=>array("rkn_id"=>10,"user"=>"Dhima","nama_penyedia"=>"Dhima Tauvik Ariezha","npwp"=>"10.2982288","alamat"=>"bogor"));
			$result = $this->load->view("blacklist",$data);
		}else{
			$result = "Somethings Wrong Happen!!";
		}
		echo $result;
	}
	...
	function delbl($id=0)
	{
		$id = $this->input->post("idbl");
		$iddh = $this->input->post("iddh");
		$del_status = $this->daftar_hitam->setDeleteBl($id,$iddh);
		$response['id'] = $id;
		$response['status'] = $del_status;
		echo $result = json_encode($response);
		exit();
