...
class admin_ref_pengadaan extends MX_Controller  {
	
	var $table = "ref_pengadaan";
	var $table_alias = "Kategori Pengadaan";
	var $uri_page = 6;
	var $per_page = 25;
	...
	function __construct()
	{
		...
		$this->load->model("admin_ref_pengadaan",'ref_pengadaan');
	...
	{	
		...
		$jum_record = $this->ref_pengadaan->getTotal($this->table,"ref_pengadaan",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->ref_pengadaan->getList($this->table,"ref_pengadaan",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->ref_pengadaan->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
	...
	function submit()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				$this->ref_pengadaan->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{
				$id_term = $this->ref_pengadaan->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->ref_pengadaan->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->ref_pengadaan->getList($this->table,null,null,null);
		$list = array();
