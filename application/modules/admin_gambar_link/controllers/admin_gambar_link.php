...
class admin_gambar_link extends MX_Controller  {
	
	var $table = "gambar_link";
	var $table_alias = "Gambar Link";
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		//Edited BY FACHMY - 26/01/2015
		$path = base_url().'admin_'.$this->table."/pages/".$sch1_parm."/".$sch2_parm."/".$per_page;
		$jum_record = $this->gambar_link->getTotal($this->table,"ref_link",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->gambar_link->getList($this->table,"ref_link",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->gambar_link->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
				foreach($q->result_array() as $r){
					...
					// 	$image = $r['file_image'];
					// 	$file_image = "
					// 	<a href='".base_url()."uploads/gambar_link/".$image."' rel='facebox'><img src='".base_url()."uploads/gambar_link/thumbs/".$image."' class='thumbnail'></a>
					// 	<br/>
	...
	function submit()
	{
		...
		if($_FILES["file"]['name']){
			if(file_exists("./uploads/gambar_link/".$file_image)){
				unlink("./uploads/gambar_link/".$file_image);
			}
			if(file_exists("./uploads/gambar_link/thumbs/".$file_image)){
				unlink("./uploads/gambar_link/thumbs/".$file_image);
			}
			$this->load->library('image_moo');
			$file_image = $file_name;
			$this->image_moo->load($file_image_tmp)->resize_crop($w,$h)->save("./uploads/gambar_link/".$file_image);
			$this->image_moo->load($file_image_tmp)->resize_crop(125,125)->save("./uploads/gambar_link/thumbs/".$file_image);
			//move_uploaded_file($file_image_tmp, "./uploads/gambar_link/documents/".$file_name);
		}else{
		...
		unset($fields['date2']);
		foreach ($fields as $field) {
			if($field!='ordered' && $field!='link' && $field!='content' && $field!='deleted_at' && $field!='is_delete' && $field!='publish' && $field!='file_image' && $field!='create_date' && $field!='modify_date' && $field!='modify_user_id' && $field!='date2' && $field!='modify_user_id' && $field!='user_id'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				$this->gambar_link->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{
				$id_term = $this->gambar_link->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->gambar_link->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function unlink($id,$file_image)
	{
		...
		if(file_exists("uploads/gambar_link/".$file_image)){ unlink("uploads/gambar_link/".$file_image); }
		if(file_exists("uploads/gambar_link/thumbs/".$file_image)){ unlink("uploads/gambar_link/thumbs/".$file_image); }
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->gambar_link->getList($this->table,null,null,null);
		$list = array();
