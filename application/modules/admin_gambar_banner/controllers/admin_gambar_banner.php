...
class admin_gambar_banner extends MX_Controller  {
	
	var $table = "gambar_banner";
	var $table_alias = "Banner Slider";
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->gambar_banner->getTotal($this->table,"ref_link",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->gambar_banner->getList($this->table,"ref_link",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->gambar_banner->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
	...
	function submit()
	{
		...
		if($_FILES["file"]['name']){
			...
			$this->image_moo->load($file_image_tmp)->resize_crop(1147,350)->save("./uploads/gambar_banner/".$file_image);
			//move_uploaded_file($file_image_tmp, "./uploads/documents/".$file_name);

			if($w!=1147 && $h!=350){
				$this->session->set_flashdata('err', 'Ukuran image harus 1147px x 350px ');
		...
		unset($fields['date2']);
		foreach ($fields as $field) {
			if($field!='link' && $field!='content' && $field!='order' && $field!='is_delete'&&  $field!='deleted_at' && $field!='publish' && $field!='file_image' && $field!='create_date' && $field!='modify_date' && $field!='modify_user_id' && $field!='date2' && $field!='modify_user_id' && $field!='user_id'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				$this->gambar_banner->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{

				$id_term = $this->gambar_banner->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->gambar_banner->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->gambar_banner->getList($this->table,null,null,null);
		$list = array();
