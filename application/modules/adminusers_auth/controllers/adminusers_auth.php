...
class adminusers_auth extends MX_Controller  {
		...
		if(is_numeric($id)){
			...
			if($q->num_rows() > 0){
				foreach($q->result() as $r){
					...
					$other_email = $this->session->flashdata("other_email") ? $this->session->flashdata("other_email") : $r->other_email;
					...
					$other_email = $this->session->flashdata("other_email") ? $this->session->flashdata("other_email") : "";
	...
	function submit()
	{
		...
		$this->form_validation->set_rules('other_email', 'other email', 'valid_email');
		if ($this->form_validation->run($this) == FALSE)
		{
			$this->session->set_flashdata("err",validation_errors());
			$this->session->set_flashdata("ref_title",$username);
			$this->session->set_flashdata("email",$email);
			$this->session->set_flashdata("other_email",$other_email);
			...
			if($id > 0)
			{
				$this->adminusers_auth->setUpdate($this->table,$id,$username,$email,$other_email,$password,$publish,$adminusers_level_id,$user_id);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$this->adminusers_auth->setInsert($this->table,$id,$username,$email,$other_email,$password,$publish,$adminusers_level_id,$user_id);
	...
	{
		$count = $this->adminusers_auth->checkUsername($this->table,$username,$this->input->post('id'));
		if ($count > 0)
		{
