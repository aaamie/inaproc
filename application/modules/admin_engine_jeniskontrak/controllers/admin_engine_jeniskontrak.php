...
class admin_engine_jeniskontrak extends MX_Controller  {
	
	var $table = "engine_jeniskontrak";
	var $table_alias = "Jenis Kontrak";
	var $uri_page = 6;
	var $per_page = 25;
	...
	function __construct()
	{
		...
		$this->load->model("admin_engine_jeniskontrak",'engine_jeniskontrak');
	...
	{	
		...
		$jum_record = $this->engine_jeniskontrak->getTotal($this->table,"engine_jeniskontrak",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->engine_jeniskontrak->getList($this->table,"engine_jeniskontrak",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->engine_jeniskontrak->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
	...
	function submit()
	{
		...
		foreach ($fields as $field) {
			if($field!='is_delete'&&  $field!='deleted_at' && $field!='order' && $field!='id_fisrt'  && $field!='publish'&&$field!='create_date' && $field!='modify_date' && $field!='modify_user_id' && $field!='date2' && $field!='modify_user_id' && $field!='user_id'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($_POST['id_first'] > 0)
			{
				...
				$this->engine_jeniskontrak->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{
				unset($postData['id_first']);
				$id_term = $this->engine_jeniskontrak->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->engine_jeniskontrak->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->engine_jeniskontrak->getList($this->table,null,null,null);
		$list = array();
