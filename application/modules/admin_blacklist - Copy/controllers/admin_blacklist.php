...
class admin_blacklist extends MX_Controller  {
	...
	{	
		...
		$jum_record = $this->blacklist->getTotal($this->table,"ref_blacklist",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->blacklist->getList($this->table,"ref_blacklist",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->blacklist->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
	...
	function submit()
	{
		...
		foreach ($fields as $field) {
			if($field!='auditupdate' && $field!='createdate'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				$this->blacklist->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->blacklist->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->blacklist->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->blacklist->getList($this->table,null,null,null);
		$list = array();
