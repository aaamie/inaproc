...
class propinsi extends MX_Controller  {
	...
	{	
		...
		$jum_record = $this->propinsi->getTotal($this->table,"ref_propinsi",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->propinsi->getList($this->table,"ref_propinsi",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->propinsi->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
	...
	function submit()
	{
		...
		$postData['prp_id']  = $postData['id']; 
		unset($postData['id']);
		# end gungun #

		$err = "";
		$audituser = strip_tags($this->input->post("audituser"));
		$audittype = $this->input->post("audittype");
		$prp_nama = $this->input->post("prp_nama");
		...
		foreach ($fields as $field) {
			if($field!='prp_id' && $field!='auditupdate'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				$this->propinsi->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->propinsi->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->propinsi->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->propinsi->getList($this->table,null,null,null);
		$list = array();
