...
class aktivitas extends MX_Controller  {
	
	var $table = "aktivitas";
	var $table_alias = "Aktivitas";
	var $uri_page = 6;
	var $per_page = 25;
	...
	{	
		...
		$jum_record = $this->aktivitas->getTotal($this->table,"ref_aktivitas",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->aktivitas->getList($this->table,"ref_aktivitas",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->aktivitas->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
	...
	function submit()
	{
		...
		foreach ($fields as $field) {
			if($field!='akt_id' && $field!='auditupdate'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($id > 0)
			{
				$this->aktivitas->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect($this->table."/edit/".$id);
			}else{
				$id_term = $this->aktivitas->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->aktivitas->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->aktivitas->getList($this->table,null,null,null);
		$list = array();
