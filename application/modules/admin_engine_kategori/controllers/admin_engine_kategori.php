...
class admin_engine_kategori extends MX_Controller  {
	
	var $table = "engine_kategori";
	var $table_alias = "Kategori";
	var $uri_page = 6;
	var $per_page = 25;
	...
	function __construct()
	{
		...
		$this->load->model("admin_engine_kategori",'engine_kategori');
	...
	{	
		...
		$jum_record = $this->engine_kategori->getTotal($this->table,"engine_kategori",$sch1_parm,$sch2_parm);
		...
		#record
		$query = $this->engine_kategori->getList($this->table,"engine_kategori",$per_page,$lmt,$sch1_parm,$sch2_parm);
		$list = array();
		if($query->num_rows() > 0){
	...
	function edit_content($id)
	{
		...
		if(is_numeric($id)){
			...
			#record
			$q = $this->engine_kategori->getDetail($this->table,$id);
			$list = array();
			if($q->num_rows() > 0){
	...
	function submit()
	{
		...
		foreach ($fields as $field) {
			if($field!='file_image' && $field!='order' && $field!='id_fisrt'  && $field!='publish'&&$field!='create_date' && $field!='modify_date' && $field!='modify_user_id' && $field!='date2' && $field!='modify_user_id' && $field!='user_id'){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($_POST['id_first'] > 0)
			{
				...
				$this->engine_kategori->setUpdate($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
				$this->session->set_flashdata("success","Data saved successful");
				redirect("admin_".$this->table."/edit/".$id);
			}else{
				unset($postData['id_first']);
				$id_term = $this->engine_kategori->setInsert($this->table,$postData,$file_image_tmp,$file_image_name,$file_image_old);
	...
	{
		$del_status = $this->engine_kategori->setDelete($this->table,$id);
		$response['id'] = $id;
	...
	function getRefDropdown($id,$name,$type=NULL)
	{
		...
		$q = $this->engine_kategori->getList($this->table,null,null,null);
		$list = array();
